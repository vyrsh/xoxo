...
int max_index(long double inp[], long double arr[])
{
    int max = 0;
    for (int i=0; i<9; i++) {
        if (inp[i]==0)
            max=i;
    } // set max to any vacant spot
    for (int i=0; i<9; i++)
    {
        printf("%Lf ", arr[i]);
        if (arr[i]>arr[max])
        {
            if (inp[i]==0) // will ignore if occupied. COMMENT to teach AI empty
                max = i;
        }
    }
    return max;
...
void ai_play(long double arr[])
{
    ...
    //printf("cost: %Lf\n", cost(layerOo, temp));
    int max = max_index(arr, layerOo);
    printf("max: %d\n", max+1);
    //printf("[%d]: %Lf\n", i+1, layer1o[i]);
    arr[max] = -1;
    return; // comment last 2 lines, to train ai to not place in occupied
    if (arr[max]==0)
    {
        arr[max] = -1;
    }
